...
local function format_current_doc(reload)
	...
	---Prettier patch (otherwise getting empty file until reset)
	io.stdout:setvbuf 'no' 
	local file = assert(io.popen(cmd .. " 2> >(head -n 1 | cut -d ':' -f2-)", 'r')) -- Running cmd (stderr=>stdo and cut for displaying)
	file:flush()  -- > important to prevent receiving partial output
	local output = file:read('*all')
	file:close()
	---End of Prettier patch
	if output then core.error(output) end
